...
function window_onclick_handler(event) {
    var mm1 = document.getElementById("id_top_nav");
...
function get_ancestors(li_elem) {
    ...
    //document.getElementById('DEBUG').innerHTML = "parents of";
    //dbg_add_li_html(li_elem);
    const all_ancestors = [];
    var ii = 0;
    var pp = li_elem;
...
function init_side_nav_closers() {
    ...
    for (ii = 0; ii < all_li.length; ii++) {
        let jj = all_li[ii];
        var all_sub_li = jj.getElementsByTagName("li");
        if (all_sub_li.length == 0){
